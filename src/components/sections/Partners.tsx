        ...
        <div className="mt-8 grid grid-cols-2 items-center gap-6 sm:grid-cols-4 lg:grid-cols-8">
          {LOGOS.map((l, i) => (
            <Reveal key={l} delay={i * 0.04}>
              <div className="flex items-center justify-center rounded-xl border border-border bg-background px-3 py-4 text-center font-display text-sm font-bold text-foreground/60 transition hover:text-primary">
                {l}
              </div>
            </Reveal>
          ))}
