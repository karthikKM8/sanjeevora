...
export function QuickDonate() {
  ...
  const [amount, setAmount] = useState<number | "custom">(1200);
  ...
  return (
    <div className="relative w-full max-w-md rounded-3xl border border-border bg-background/95 p-6 shadow-2xl shadow-primary/10 backdrop-blur sm:p-7">
          ...
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
            {PRESETS.map((p) => (
              <button
                ...
                key={p}
                onClick={() => setAmount(p)}
                className={`rounded-xl border px-3 py-2.5 text-sm font-semibold transition ${amount === p
            ...
            <button
              ...
              className={`rounded-xl border px-3 py-2.5 text-sm font-semibold transition ${amount === "custom"
                  ? "border-primary bg-primary text-white shadow"
                  : "border-border bg-background hover:border-primary/50"
                }`}
            >
